...
def save_point_clouds(original_pts, reconstructed_pts, i):
    for name, pc in zip(['original', 'reconstructed'], [original_pts, reconstructed_pts]):
...
class Point_M2AE(nn.Module):
    ...
    def forward(self, pts, eval=False, **kwargs):
        ...
        if True:
            #save point clouds
            L, _ = x_full.reshape(-1, C).shape
            rec_points = self.rec_head(x_full.reshape(-1, C).unsqueeze(-1)).reshape(L, -1, 3) + centers[-2].reshape(L, -1, 3)
            gt_points = neighborhoods[-2].reshape(L, -1, 3) + centers[-2].reshape(L, -1, 3)

            save_point_clouds(gt_points, rec_points, i=kwargs.get('i', 0))
        return loss
